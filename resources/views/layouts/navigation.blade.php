<nav class="bg-white shadow-sm px-3 py-2 sm:px-6 sm:py-3 flex items-center justify-between w-full z-30">
        ...
        <div class="hidden lg:flex items-center gap-2 ml-6">
            ...
            @if($user)
                @if($user->isOwner())
                    ...
                    <a href="{{ route('orders.index') }}" class="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('orders.*') ? 'bg-blue-100 text-blue-700 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-blue-600' }}">
            ...
            <div class="p-4 space-y-2">
                ...
                @if($user)
                    @if($user->isOwner())
                        ...
                        <a href="{{ route('orders.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-700 hover:bg-blue-50 transition-colors {{ request()->routeIs('orders.*') ? 'bg-blue-100 text-blue-700' : '' }}">
    ...
    <div class="flex items-center gap-2 sm:gap-3">
        ...
        <a href="{{ route('orders.index') }}" class="relative hover:bg-blue-50 p-2 rounded-lg transition-all duration-200 group" title="My Orders" aria-label="View my orders">
            <svg class="h-5 w-5 text-gray-500 group-hover:text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            @if($pendingOrdersCount > 0)
                <span class="absolute -top-1 -right-1 bg-blue-500 text-white text-xs rounded-full px-1.5 py-0.5 font-bold">{{ $pendingOrdersCount }}</span>
            @endif
        </a>
