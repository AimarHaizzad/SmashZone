    ...
    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('courts.store', absolute: false) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="location" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-300 focus:border-blue-500 transition-colors text-lg">
                            ...
                            <option value="middle" {{ old('location') == 'middle' ? 'selected' : '' }}>Middle - Center of facility</option>
                            <option value="edge" {{ old('location') == 'edge' ? 'selected' : '' }}>Edge - Side of facility</option>
                            <option value="corner" {{ old('location') == 'corner' ? 'selected' : '' }}>Corner - Corner position</option>
                            <option value="center" {{ old('location') == 'center' ? 'selected' : '' }}>Center - Central area</option>
                            <option value="side" {{ old('location') == 'side' ? 'selected' : '' }}>Side - Side area</option>
                            <option value="front" {{ old('location') == 'front' ? 'selected' : '' }}>Front - Near entrance</option>
                            <option value="back" {{ old('location') == 'back' ? 'selected' : '' }}>Back - Rear area</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-6 border border-yellow-100">
                ...
                <p class="text-sm text-gray-600 mb-4">Set different hourly rates for various time ranges. These prices will be used when calculating bookings.</p>
            ...
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-100">
                ...
                <div class="space-y-2">
                    ...
                    <select name="status" 
                        ...
                        <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>Closed - Not available</option>
                    </select>
                    @error('status')
                        <div class="text-red-500 text-sm flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
