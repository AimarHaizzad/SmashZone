...
<div class="max-w-xl mx-auto py-8">
    ...
    <form action="{{ route('bookings.store') }}" method="POST" class="bg-white shadow rounded p-6">
        ...
        <div class="mb-4">
            ...
            <input type="time" name="start_time" class="w-full border rounded px-3 py-2" value="{{ old('start_time') }}" required>
            @error('start_time')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1 font-semibold">End Time</label>
            <input type="time" name="end_time" id="end_time" class="w-full border rounded px-3 py-2" value="{{ old('end_time') }}" required>
